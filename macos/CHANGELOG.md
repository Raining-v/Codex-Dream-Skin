...
- 修复全新下载后默认主题仍引用本机临时图片、导致安装与自检失败的问题；现在使用仓库内置的红白科幻横幅
- 修正外观配置测试：安装只备份相关键，不再错误要求把 `appearanceTheme` 强制改成暗色

### 改进

- 增加可跨平台运行的默认素材与配置往返测试，发版前即可发现缺图和配置误改
- 同步运行说明、QA 版本与客户交付版本文案
